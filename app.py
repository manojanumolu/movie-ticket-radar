...
def sidebar(active_count: int) -> str:
    ...
    with st.sidebar:
        ...
        C.html(
            ...
            "</div>"
            # The rail's ambience: Home's own reel drawing, twice, with a run
            # of film between them. Carried in this block so it adds no
            # element (and no gap) to the rail; the theme fixes it behind the
            # navigation and clips it to the rail (``.tr-side-ambience``).
            '<div class="tr-side-ambience" aria-hidden="true">'
            + home.reel_svg("top") + '<div class="tr-side-film"></div>' + home.reel_svg("bottom")
            + "</div>"
        )
        return choice
